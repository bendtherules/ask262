...
async function main() {
  ...
  const queryEngineTool = new QueryEngineTool({
    ...
    metadata: {
      name: "spec_retriever",
      description: "Queries the language specification for text content about specific sections or topics. Use this to get the detailed text of a section.",
    },
  });
  ...
  const graphTool = {
    metadata: {
      name: "graph_explorer",
      description: "Explores structural relationships between specification sections and implementation code (functions). Use this to find which spec section a function implements. Input: section ID or function name.",
      parameters: {
        type: "object",
        properties: {
          query: {
...
6. If the tools do not provide enough information, state that clearly rather than guessing from your internal knowledge.`
  ...
  const message = process.argv[2] || "Which spec section does Evaluate_IfStatement implement? and what does that section say?";
